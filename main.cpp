...
int main(int argc, char *argv[]){
	...
	fprintf(outprof,"# r_in r_mid r_out [Mpc] ang_in ang_mid ang_out [arcsecs] Lx [ergs/s] kT [keV] n_gas [cm^-3]\n");
    }
    ...
    //for( i=0; i < 100; i++){
        // cout << "Computing SB profile for halo " << i << endl;
        halo = &(halos->list[i]);
	...
	if (strcmp(file_format,"rockstar")==0) {
	    fprintf(outhalo,"%ld %d %f %f %f %f %f %f %f %e %e %e %f \n",halo->id,halo->pid,halo->x,halo->y,halo->z,halo->redshift,Rvir,Rscale,R500,Mvir,halo->M200c/h,halo->M500c/h,halo->Xoff);
	}
	else if (strcmp(file_format,"lightcone")==0) {
	...
	if (strcmp(file_format,"simple")==0){
		...
		fprintf(outprof,"%f %f %f %f %f %f %e %e %e\n", r_in[j], rbins[j], r_out[j], ang_in[j], angbins[j], ang_out[j], Lx[j], kT[j], ngas[j]);
	    }
        }
	if (strcmp(file_format,"simple")==0) fclose(outprof);
    }
    if (strcmp(file_format,"simple")!=0){
	fclose(outhalo);
	fclose(outprof);		
